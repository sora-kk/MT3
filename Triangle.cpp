...
void Triangle::DrawTriangle(const TriangleData &triangle, const Matrix4x4 &viewProjectionMatrix, const Matrix4x4 &viewportMatrix, uint32_t color) {
   // 各頂点をワールド座標からスクリーン座標に変換
    ...
    transformed[0] = vector.Transform(vector.Transform(triangle.vertices[0], viewProjectionMatrix), viewportMatrix);
    transformed[1] = vector.Transform(vector.Transform(triangle.vertices[1], viewProjectionMatrix), viewportMatrix);
    transformed[2] = vector.Transform(vector.Transform(triangle.vertices[2], viewProjectionMatrix), viewportMatrix);
